...
Проект разрабатывается с помощью трёх ИИ-ролей, каждая из которых реализована как
Cursor Skill (`.cursor/skills/`). Роли имеют чёткие границы ответственности и не
пересекаются.

| Роль | Cursor Skill | Ответственность | Выходные артефакты |
|---|---|---|---|
| Владелец продукта | `.cursor/skills/product-owner/` | Гейм-дизайн, UX, бизнес-фичи. Думает об игре с точки зрения игрока. | Обновления `docs/rules.md`, бизнес-описания фич. |
| Архитектор-аналитик | `.cursor/skills/architect/` | Техническое проектирование, ТЗ, ADR. Переводит бизнес-требования в технические спецификации. | Feature specs, обновления `data-models.md`, `api-contract.md`, ADR в `docs/adr/`. |
| Разработчик | `.cursor/skills/developer/` | Реализация фич по ТЗ, поддержание кода и его документации. | Код, обновление индексов фич. |

Кроме ролей, проект использует **Cursor Rules** (`.cursor/rules/`) — автоматически
подключаемый контекст:

| Правило | Файл | Когда активно |
|---|---|---|
| Контекст проекта | `.cursor/rules/project-context.mdc` | Всегда |
| Go конвенции | `.cursor/rules/go-backend.mdc` | При работе с `backend/**/*.go` |
| React/TS конвенции | `.cursor/rules/react-frontend.mdc` | При работе с `frontend/**/*.{ts,tsx}` |
| Стандарты документации | `.cursor/rules/documentation.mdc` | При работе с `**/docs/**/*.md` |

## Жизненный цикл фичи

```mermaid
flowchart TD
    PO["Владелец продукта:<br/>описать бизнес-фичу"]
    PO --> AR["Архитектор:<br/>спроектировать решение"]
    AR --> SPEC["Архитектор:<br/>написать feature spec"]
    SPEC --> CHECK["Вы: проверить спек"]
    CHECK --> DEV["Разработчик:<br/>реализовать по спеку"]
    DEV --> REVIEW["Вы: ревью кода"]
    REVIEW --> OK{Принято?}
    OK -->|Нет| FB["Вы: замечания"]
    FB --> DEV
    OK -->|Да| MERGE["Вы: влить код"]
    MERGE --> DOCS["Обновить документацию"]
    DOCS --> DONE["Пометить фичу как done"]
```

### Шаги подробно

**1. Владелец продукта: описать бизнес-фичу.**
Вы формулируете идею, ИИ в роли Product Owner помогает её проработать: валидирует
против текущих правил (`docs/rules.md`), описывает с точки зрения игрока, обновляет
правила при необходимости.

**2. Архитектор: спроектировать решение.**
ИИ в роли Architect-Analyst читает бизнес-описание, анализирует текущую архитектуру
и проектирует техническое решение: модели данных, API, затронутые файлы. При необходимости
создаёт ADR в `docs/adr/`.

**3. Архитектор: написать feature spec.**
...
**4. Вы: проверить спек.**
Спек считается самодостаточным, если ИИ-агент в роли Developer, не имеющий предыдущего
контекста, может реализовать фичу, прочитав только сам спек и указанные в нём ссылки.

**5. Разработчик: реализовать по спеку.**
Пример промпта:
...
Cursor автоматически подключит конвенции кодирования (Go или React/TS) через правила.
...
| ADR | `docs/adr/` | Записи архитектурных решений. Шаблон и индекс — в `docs/adr/README.md`. |
